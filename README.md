...
- MySQL database
- Postgres database
  > Both databases have a common name, called _ikhospital_ and they said, all tables within this one, could be used to get insights.
...
- Libraries used, mentioned on the [_requeriments.txt_](https://github.com/doug-pires/Lakehouse-MedIKal-Healthcare/blob/main/requirements.txt) file.
...
- All the **Business Logic** I built, e.g:
  ...
  - Everything is [available](https://github.com/doug-pires/Lakehouse-MedIKal-Healthcare/tree/main/Files) on the CSV files that I imported as an example.
...
- Once the IT manager said to us, they used a common name to the database called _ikhospital_ then I create the code to get all the tables from the databases and push to the ADLS Gen2.
- I used three parameters, _datasource_mysql_, _datasource_postgres_ and _database_ and these parameters I used to create the **Structure on Bronze container**. More about the structure on the next session, Storage Account.
...
- Bronze container, we are going to land our data coming from the datasources, following the structure like that:
  - _container/datasource/databaseORschema/tables_ becoming that _bronze/mysql/ikhospital/tables_
...
- _container/databaseORschema/tables_ becoming that _silverORgold/ikhospital/tables_
...
- The serving layer, the option was SQL Serverless Pool. We can create EXTERNAL TABLES or VIEWS on top of our data on the ADLS Gen2. To serve the data to Power BI, I created the database called _IKHOSPITAL_DW_ then we can generate our table on that one.
...
- To the Power BI Developer read the tables is necessary an user. Then I created one user called **PBI_Reader**, I gave the right credentials, GRANTS to SELECthe DATABASE _IKHOSPITAL_DW_ and the user can get/query the tables on Power BI.
...
- We can use SQL Authentication OR AAD authentication.
  > Keep in mind, the tables are small that's why we choose _import mode_, however if we are discussing a large dataset, always a good idea thinking about other methods, like _Direct Query_ with _Incremental refreshs_, since our data is stored as **Delta tables**
